  ...
  handleSelectQuery(results) {
    ...
    // Queries with include
      ...
      results = AbstractQuery._groupJoinData(results, {
        ...
        checkExisting: this.options.nested ? false : this.options.hasMultiAssociation
      });
    ...
    if (this.options.nested && this.options.includeNames) {
      const grouped = [];
      const keys = {};

      result.forEach(row => {
        if (typeof keys[row[this.model.primaryKeyAttribute]] === 'undefined') {
          keys[row[this.model.primaryKeyAttribute]] = grouped.length;
          grouped.push(row);
          return;
        }

        const prev = grouped[keys[row[this.model.primaryKeyAttribute]]];

        this.options.includeNames.forEach(prop => {
          if (Array.isArray(prev.dataValues[prop]) && row[prop]) {
            prev[prop] = prev.dataValues[prop] = prev.dataValues[prop].concat(row[prop]);
          }
        });
      });
